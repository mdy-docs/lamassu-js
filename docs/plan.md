...
| 12 | ✅ `Error` built-in (`src/js_error.c`): `Error`/`TypeError`/`RangeError`/`ReferenceError`/`SyntaxError` constructors (callable with or without `new`, `options.cause`), real prototypes with `name`/`message`/`toString`, `JS_OBJ_ERROR` rendered as `name: message` by ToString, and every engine-raised error routed through `js_error_from_ascii` so `catch (e) { e.name; e.message }` is uniform (bare-string fallback only on OOM) | 1 |
